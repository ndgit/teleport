...
- Teleport version `8.0` or higher.
...
with CockroachDB. This means that:

- Teleport must trust certificates presented by CockroachDB nodes.
- CockroachDB must trust client certificates signed by Teleport.
...
and `node.crt` / `node.key` with the node's certificate and key. Do this for
every cluster node and make sure to use the hostname Teleport will be using to
connect to the nodes in the `--host` flag.

<Admonition type="note">
  CockroachDB expects CA, certificate and key to be named exactly how `tctl auth sign`
  command names then. See [Node key and certificates](https://www.cockroachlabs.com/docs/v21.1/create-security-certificates-custom-ca#node-key-and-certificates)
  for details.
</Admonition>
